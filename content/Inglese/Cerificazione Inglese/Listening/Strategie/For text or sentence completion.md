...
// ProprietÃ  specifiche degli input
...
parent.append(div, "testo", altroEl);     // uno o piÃ¹ nodi/stringhe
...
// DocumentFragment â€” minimizza i reflow
...
el.classList.add("grande", "visibile");    // piÃ¹ classi
...
el.style.cssText = "color: red; font-size: 1rem;"; // piÃ¹ stili in una volta
...
// ProprietÃ  riflesse (piÃ¹ comodo per attributi standard)
...
el.parentNode              // genitore (Node, puÃ² essere document)
el.closest(".wrapper")     // antenato piÃ¹ vicino che corrisponde al selettore
...
function handler(event) {
  ...
  event.currentTarget;    // elemento su cui Ã¨ il listener
  ...
  // Tastiera
  event.key       // "Enter", "ArrowUp", "a", "A"â€¦
  event.code      // "KeyA", "Enter", "Space"â€¦
  ...
  event.inputType         // "insertText", "deleteContentBackward"â€¦
}
...
lista.addEventListener("click", (event) => {
  const item = event.target.closest("li");  // trova il <li> piÃ¹ vicino
...
const observer = new IntersectionObserver((entries) => {
  for (const entry of entries) {
    if (entry.isIntersecting) {
      ...
      console.log("VisibilitÃ :", entry.intersectionRatio);
    }
  }
}, {
  root: null,               // null = viewport
  rootMargin: "0px 0px -100px 0px", // margini del root
  threshold: [0, 0.5, 1],  // callback al 0%, 50%, 100% di visibilitÃ 
});
...
const observer = new ResizeObserver((entries) => {
  for (const entry of entries) {
    ...
    console.log(`${entry.target.id}: ${width}Ã—${height}`);
...
// sessionStorage â€” stessa API di localStorage
...
// Attributi
// expires=DATE   â€” data scadenza (formato UTC)
// max-age=N      â€” secondi dalla creazione
// path=/         â€” percorso valido
// domain=.example.com â€” dominio e sottodomini
// secure         â€” solo HTTPS
// samesite=Strict|Lax|None â€” protezione CSRF
// httponly       â€” non accessibile via JS (solo server)
...
```js
// setTimeout â€” esegue una volta dopo N ms
const id = setTimeout(() => {
  console.log("Eseguito dopo 2 secondi");
}, 2000);
clearTimeout(id); // annulla

// setInterval â€” esegue ogni N ms
...
// Timeout "zero" â€” mette in fondo alla coda degli eventi
setTimeout(() => console.log("dopo"), 0);
console.log("prima"); // "prima" â†’ "dopo"
...
```js
// â”€â”€ Thread principale â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ worker.js â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// SharedWorker â€” condiviso tra piÃ¹ tab
...
console.log("%O", oggetto);     // proprietÃ  dell'oggetto
...
console.assert(1 === 1, "Non verrÃ  stampato");
console.assert(1 === 2, "1 non Ã¨ uguale a 2"); // stampa errore
...
// structuredClone (ES2022) â€” supporta Date, Map, Set, ArrayBuffer, ecc.
...
// { data: Date, numeri: Set } â€” correttamente clonati
...
// ora buffer Ã¨ "neutered" (trasferito, non piÃ¹ usabile)
```
...
```js
// Debounce â€” esegue solo dopo N ms dall'ultima chiamata
...
// Throttle â€” esegue al massimo una volta ogni N ms
...
// Con WeakMap (per oggetti â€” garbage collectable)
